...
test('or', async () => {
  const res = await postgrest
    ...
    .or('status.eq.OFFLINE,username.eq.powerbot')
  expect(res).toMatchInlineSnapshot(`
    Object {
      "count": null,
      "data": Array [
...
test('eq', async () => {
  const res = await postgrest.from('users').select('username').eq('username', 'powerbot')
  expect(res).toMatchInlineSnapshot(`
    Object {
      "count": null,
      "data": Array [
...
test('neq', async () => {
  const res = await postgrest.from('users').select('username').neq('username', 'powerbot')
  expect(res).toMatchInlineSnapshot(`
    Object {
      "count": null,
      "data": Array [
...
test('like', async () => {
  const res = await postgrest.from('users').select('username').like('username', '%power%')
  expect(res).toMatchInlineSnapshot(`
    Object {
      "count": null,
      "data": Array [
...
test('ilike', async () => {
  const res = await postgrest.from('users').select('username').ilike('username', '%POWER%')
  expect(res).toMatchInlineSnapshot(`
    Object {
      "count": null,
      "data": Array [
...
test('ilikeAnyOf', async () => {
  const res = await postgrest
    .from('users')
    .select('username')
    .ilikeAnyOf('username', ['%power%', '%KIWI%'])
  expect(res).toMatchInlineSnapshot(`
    Object {
      "count": null,
      "data": Array [
...
test('filter', async () => {
  const res = await postgrest.from('users').select('username').filter('username', 'eq', 'powerbot')
  expect(res).toMatchInlineSnapshot(`
    Object {
      "count": null,
      "data": Array [
...
test('match', async () => {
  const res = await postgrest
    ...
    .match({ username: 'powerbot', status: 'ONLINE' })
  expect(res).toMatchInlineSnapshot(`
    Object {
      "count": null,
      "data": Array [
...
test('filter on rpc', async () => {
  const res = await postgrest
    .rpc('get_username_and_status', { name_param: 'powerbot' })
    .neq('status', 'ONLINE')
  expect(res).toMatchInlineSnapshot(`
    Object {
      "count": null,
